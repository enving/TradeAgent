...
class NewsAggregator:
    ...
    def __init__(self):
        self.finnhub_client = (
            finnhub.Client(api_key=config.FINNHUB_API_KEY) if config.FINNHUB_API_KEY else None
        )
        self.newsapi_client = (
            NewsApiClient(api_key=config.NEWS_API_KEY) if config.NEWS_API_KEY else None
        )
    ...
    async def _fetch_finnhub(self, ticker: str, days: int) -> List[NewsArticle]:
        ...
        try:
            ...
            news_data = await loop.run_in_executor(
                None, lambda: client.company_news(ticker, _from=start_date, to=end_date)
            )
